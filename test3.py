...
def check_vertical_line(points):
    ...
    horizontal_variation = max_x - min_x
    
    return distance_y > 200 and horizontal_variation < 100
...
while cap.isOpened():
    ...
    cv2.putText(frame, "Spell 2: Draw circles with both hands", (10, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(frame, "Spell 3: Draw horizontal line", (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.putText(frame, "Spell 4: Draw vertical line with one finger", (10, 120),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    ...
    if results.multi_hand_landmarks:
        ...
        if not gesture_detected and len(path_points) > 15:
            ...
            # 檢查單手畫垂直線
            if check_vertical_line(list(path_points)) and num_hands == 1:
                print("咒語四成功")
                cv2.putText(frame, "Spell 4 Success!", (200, 200), 
                           cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 255, 0), 3)
                cooldown = COOLDOWN_FRAMES
